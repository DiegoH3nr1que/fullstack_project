          ...
          <section className="container max-w-screen-lg mx-auto py-10">
                  ...
                  <div key={game.id} className="relative">
                    ...
                    <div className="absolute bottom-0 left-0 right-0 bg-black bg-opacity-50 p-4 rounded-b-lg">
                      ...
                      <div className="flex justify-between items-center mt-2">
                        ...
                        <button onClick={() => handleLinkClick(`/games/${game.slug}`)} className="mt-4 inline-block bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-4 rounded-md">
                          Detalhes
                        </button>
                      </div>
                    </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
              {currentReleasesGames.map((game, index) => (
                <div 
                  key={index} 
                  onClick={() => handleLinkClick(`/games/${game.slug}`)} 
                ...
                >
                  <div className="relative flex-1">
                    <Image
                      ...
                      layout="fill"
                      objectFit="cover"
                      className="rounded-t-lg transition-transform duration-300 w-full h-full"
                    />
                  </div>
                  <div className="bg-black bg-opacity-75 p-4 rounded-b-lg text-white flex flex-col justify-between h-1/3 relative">
                    <div className="relative">
                      <h3 className="text-xl font-bold truncate">
                        {game.name}
                      </h3>
                      <div className="absolute left-0 bottom-full mb-2 hidden group-hover:block bg-gray-900 text-white text-sm p-2 rounded shadow-lg w-48">
                        {game.name}
                      </div>
                    </div>
                    <p className="text-sm">{formatDate(game.released)}</p>
